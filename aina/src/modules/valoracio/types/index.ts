// Valoració d'Ofertes TypeScript types
...
// Evaluation result for frontend
...
export const SUPPORTED_FILE_TYPES = ["application/pdf", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/msword", "text/plain"] as const
...
export function getDisplayName(companyName: string | null, proposalName: string): string {
...
export function getShortDisplayName(companyName: string | null, proposalName: string): string {
	if (companyName && companyName.trim().length > 0) {
		return companyName.length > 30 ? `${companyName.substring(0, 27)}...` : companyName
	}

	const shortName = proposalName.length > 20 ? `${proposalName.substring(0, 17)}...` : proposalName
	return `${shortName} (no identificada)`
}

export function hasCompanyInfo(evaluation: LotEvaluation): boolean {
	return evaluation.companyName !== null && evaluation.companyName.trim().length > 0
}

export function getCompanyNameOrDefault(companyName: string | null, defaultText: string = "Empresa no especificada"): string {
	return companyName && companyName.trim().length > 0 ? companyName : defaultText
}
