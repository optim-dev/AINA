...
| --------------------- | ------------------------------------------------ | ---------------------------------------- |
| **Textos processats** | No s'emmagatzemen els textos complets a BigQuery | Només mètriques i hash del text          |
| **Usuaris**           | Identificador intern (Firebase UID)              | No es guarda informació PII als logs     |
| **Sessions**          | ID aleatori temporal                             | `session_{timestamp}_{random}`           |
| **Documents**         | Hash SHA-256 del contingut                       | Per deduplicació sense guardar contingut |

**Implementació del hash de text:**

```typescript
function hashText(text: string): string {
	return crypto.createHash("sha256").update(text).digest("hex").substring(0, 16) // Truncat per privacitat
}
```
...
| --------------- | -------- | ---------------------------- | ------------------------------ |
| **LLM Logs**    | 90 dies  | Prompts poden contenir dades | Hash del text, no text complet |
